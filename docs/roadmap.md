...
- [x] Перейти на пер-орендаторський connection string (Finbuckle store/provider) або задокументувати single-DB стратегію.
- [x] Перевірити всі ентіті на наявність `TenantId` та глобальних фільтрів (в т.ч. майбутні таблиці Reporting/Outbox).
- [x] Перевірити виставлення `TenantId` на вставках (інтерсептор/`SaveChanges`).

Artifacts/зміни:
- [x] `src/Hosts/WebHost/Program.cs` — наразі single-DB з InMemoryStore/StaticStrategy (пер-tenant буде додано пізніше за потреби).
- [x] `CatalogDbContext/CoreDbContext` — глобальні фільтри підтверджено; додано інтерсептори виставлення `TenantId`.
- [x] Catalog: підтверджено `TenantId` (shadow property) у `ServiceAccount`, `UrlItem`, `Site`, `UrlBatch`.
...
- [x] `Submission.Worker` — сцена приймання/валідації батчів URL-ів (якщо не робиться у WebHost), постановка в чергу.
- [x] `Crawler.Worker` — підготовка/верифікація URL-ів (за потреби), маркування.
- [x] `Inspection.Worker` — валідація статусів, повторні спроби, аналіз помилок.
- [x] Політики retry/backoff (Polly) для зовнішніх викликів.
- [x] Експонування метрик (кількість оброблених, помилки, латентність), логи з кореляцією.

Artifacts/зміни:
- [x] DI у відповідних `Program.cs`, впровадження сервісів/репозиторіїв.
- [x] OpenTelemetry + Serilog у воркерах.
